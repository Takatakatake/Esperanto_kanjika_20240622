...
if uploaded_file is not None:
    ...
    RR={}
    # 辞書をコピーする
    QQ_copy = QQ.copy()##2度繰り返し実行してはいけない。
        ...
        if j[2]==20000:##2文字で漢字化するやつ##len(i)<=2:#1文字は存在しないはずではある。
            ...
            if j[2]==60000 or j[2]==50000 or j[2]==40000 or j[2]==30000:##文字数が比較的少なく(<=5)、実際に漢字化するエスペラント語根(文字数×10000)のみを対象とする 
                ...
                if "名詞" in j[1]:##名詞については形容詞、副詞と違い、漢字化しないものにもoをつける。
                    for k in ["o"]:
                        if not i+k in QQ:
                            RR[i+k]=[j[0]+k,j[2]+len(k)*10000-4000]#実質3000#既存でないものは優先順位を大きく下げる→普通の品詞接尾辞が既存でないという言い方はおかしい気がしてきた。(20240612)
                if "形容詞" in j[1]:
                    for k in ["a"]:
                        if not i+k in QQ:
                            RR[i+k]=[j[0]+k,j[2]+len(k)*10000-4000]#実質3000
                if "副詞" in j[1]:
                    for k in ["e"]:
                        if not i+k in QQ:
                            RR[i+k]=[j[0]+k,j[2]+len(k)*10000-4000]#実質3000
                if "動詞" in j[1]:
                    for k1,k2 in verb_suffix_2l_2.items():
                        if not i+k1 in QQ:
                            RR[i+k1]=[j[0]+k2,j[2]+(len(k1))*10000-2000]##実質5000
                    for k in ["u ","u!","i "]:##動詞の"u","i"単体の接尾辞は後ろが空白と決まっているので、2文字分増やすことができる。
                        if not i+k in QQ:
                            RR[i+k]=[j[0]+k,j[2]+(len(k))*10000-2000]##実質5000
    ...
    for i in never_used_as_roots_only:
        RR[i]=[i,len(i)*10000-2000]##これらについては数字の大きさはそこまで重要ではない
    ...
    # RR['farigx'][1]=len('farigx')*10000+8000##優先順位だけ変更

    x='mondo/n'
    RR[x.replace('/', '')]=[safe_replace(x,replacements).replace("</rt></ruby>","%%%").replace('/', '').replace("%%%","</rt></ruby>"),  len(x.replace('/', ''))*10000-7000]

    ##正しく語根分解・漢字変換してほしいやつ
    y1=[['gvid/a/nt/o',73000],['am/as',33000],["kor/a/n",43000],["ink/a",33000],["post/e/n",53000],["per/e",33000],["fer/o",33000],["kor/e",33000],["mal/a/j",43000],["mal/e",33000],
        ["par/a/n",33000],['sam/o',33000],['sat/a/n',33000],['sav/oj',43000],['sud/a/n',43000],['vet/o',33000],['ir/is',33000],['regul/us',63000],['akir/a/nt',63000],["prem/is",53000],
        ...
        ,["spir/ant",63000],["ten/is ",53000],["trakt/at",63000],["far/igx",53000]]##["pi/a/n",38000] anをa/n/で分けるのは正しくはないが。
    for i in y1:
    ...
    ##以下は完全手作業
    RR['dat/um/i'.replace('/', '')]=[safe_replace('dat/um/i',replacements).replace("</rt></ruby>","%%%").replace('/', '').replace("%%%","</rt></ruby>"),  len('dat/um/i'.replace('/', ''))*10000-7000]
    RR['dat/um/u'.replace('/', '')]=[safe_replace('dat/um/u',replacements).replace("</rt></ruby>","%%%").replace('/', '').replace("%%%","</rt></ruby>"),  len('dat/um/u'.replace('/', ''))*10000-7000]
    RR['dat/um/u!'.replace('/', '')]=[safe_replace('dat/um/u!',replacements).replace("</rt></ruby>","%%%").replace('/', '').replace("%%%","</rt></ruby>"),  len('dat/um/u!'.replace('/', ''))*10000-7000]
    #dat/um/u  dat/um/u!
    RR['tra/met/i'.replace('/', '')]=[safe_replace('tra/met/i',replacements).replace("</rt></ruby>","%%%").replace('/', '').replace("%%%","</rt></ruby>"),  len('tra/met/i'.replace('/', ''))*10000-7000]
    RR['tra/met/u'.replace('/', '')]=[safe_replace('tra/met/u',replacements).replace("</rt></ruby>","%%%").replace('/', '').replace("%%%","</rt></ruby>"),  len('tra/met/u'.replace('/', ''))*10000-7000]
    RR['tra/met/u!'.replace('/', '')]=[safe_replace('tra/met/u!',replacements).replace("</rt></ruby>","%%%").replace('/', '').replace("%%%","</rt></ruby>"),  len('tra/met/u!'.replace('/', ''))*10000-7000]
